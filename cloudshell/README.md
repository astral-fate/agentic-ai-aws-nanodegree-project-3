...
1. **Materialise** — writes every embedded project file into
   `~/novamart-project`.
2. **Preflight** — identity, region, whether the orchestrator/worker
   ...
   ```
3. **CloudFormation** — deploys `infrastructure/starter_stack.yaml` as stack
...
4. **Seed data** — `infrastructure/seed_data.py`: customers, orders, and the
   three policy documents uploaded to S3.
5. **Knowledge Bases ×3** — the phase with no precedent in the sibling
   ...
   `COMPLETE`.
6. **Deploy the agent** — `src/agent_orchestrator.py deploy`, which itself
...
7. **Grade** — prefers a workspace-provided `tests/test_agent.py` (copied in
   before the run) over the embedded copy, prints which one it used, then
   runs `python3 tests/test_agent.py all` and tees the output to
   `evidence/live/pytest_output.txt`. The grader's own exit code is always 0
   on a completed run, so the summary reads the printed `Score: X/Y` line
   rather than trusting the exit code alone.
8. **Adversarial suite** — `scripts/run_adversarial.py`, added by a later
...
9. **Package** — a minimal `~/novamart-submission.zip` (source, tests,
